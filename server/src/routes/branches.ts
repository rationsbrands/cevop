...
// GET / — list all branches for org (ADMIN/SUPERADMIN)
branchesRouter.get('/', requireRole('ADMIN', 'SUPERADMIN'), async (req: AuthRequest, res: Response) => {
  try {
...
branchesRouter.get('/:branchId', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!['SUPERADMIN', 'ADMIN', 'BRANCH_ADMIN'].includes(user.role)) {
...
// DELETE /:branchId — deactivate a branch (ADMIN/SUPERADMIN only)
branchesRouter.delete('/:branchId', requireRole('ADMIN', 'SUPERADMIN'), async (req: AuthRequest, res: Response) => {
  try {
    const { branchId } = req.params;
