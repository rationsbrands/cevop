...
serviceRequestsRouter.post('/public', async (req: Request, res: Response) => {
  try {
    ...
    if (actualBranchId) io.to(`${table.organizationId}:${actualBranchId}`).emit('SERVICE_REQUESTED', request);
...
serviceRequestsRouter.patch('/:id/status', requireRole('ADMIN', 'SUPERADMIN', 'BRANCH_ADMIN', 'WAITER', 'SERVICE'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (request.branchId) io.to(`${req.user!.organizationId}:${request.branchId}`).emit('SERVICE_REQUEST_UPDATED', request);
