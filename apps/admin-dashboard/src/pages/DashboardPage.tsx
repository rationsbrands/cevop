...
function StatCard({ label, value, sub }: { label: string; value: string | number; sub?: string }) {
  return (
    <div className="card p-5 flex flex-col justify-between min-h-[140px]">
      <div>
        <p className="text-[10px] text-[var(--muted)] uppercase tracking-widest font-extrabold mb-1">{label}</p>
        <p className="font-display text-2xl sm:text-3xl text-[var(--accent)] break-words leading-tight">{value}</p>
      </div>
      {sub && <p className="text-[11px] text-[var(--muted)] mt-2 font-medium">{sub}</p>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-6 animate-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard label="Total Revenue" value={formatPrice(summary?.totalRevenue ?? 0)} sub="All time" />
        <StatCard label="Top Item" value={summary?.popularItems?.[0]?.menuItem?.name ?? "—"} sub={(summary?.popularItems?.[0]?.totalQuantity ?? 0) + " sold"} />
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 card">
          <div className="card-header"><h2 className="font-semibold text-sm">Recent Orders</h2></div>
          <div className="overflow-x-auto">
            <table>
              <thead><tr><th>Order ID</th><th>Table</th><th>Items</th><th>Total</th><th>Status</th><th>Time</th></tr></thead>
              <tbody>
                {recentOrders.length === 0 && <tr><td colSpan={6} className="text-center text-[var(--muted)] py-6">No orders yet</td></tr>}
                {recentOrders.map((o) => (
                  <tr key={o.id}>
                    ...
                    <td className="text-[var(--muted)] text-xs">{new Date(o.createdAt).toLocaleTimeString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card">
          <div className="card-header"><h2 className="font-semibold text-sm">Top Items</h2></div>
          <div className="card-body space-y-3">
            {(summary?.popularItems ?? []).length === 0 && <p className="text-[var(--muted)] text-sm">No data yet</p>}
            {(summary?.popularItems ?? []).map((item, i) => (
              <div key={i} className="flex items-center justify-between">
                ...
                <span className="text-xs text-[var(--accent)] font-bold">{item.totalQuantity} sold</span>
              </div>
            ))}
          </div>
        </div>
