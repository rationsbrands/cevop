...
export function HelpOptionsPage() {
  ...
  useEffect(() => { load(); }, [activeBranchFilter, me?.branchId]);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      setShowForm(false);
      setEditingId(null);
      setForm({ type: 'SERVICE', label: '', icon: '', sortOrder: 0, isActive: true, branchId: activeBranchFilter?.id || me?.branchId || '' });
      load();
    } catch { setSaveError('Failed to save help option'); }
  ...
  async function handleDelete(id: string) {
    ...
    } catch { alert('Failed to delete help option'); }
  }
    ...
    <div className="space-y-6 animate-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[var(--muted)] text-sm">Configure what customers see in the "Need Help?" section.</p>
        </div>
        <button
          onClick={() => {
            setEditingId(null);
            setForm({ type: 'SERVICE', label: '', icon: '', sortOrder: options.length, isActive: true, branchId: activeBranchFilter?.id || me?.branchId || '' });
            setShowForm(true);
          }}
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider ${opt.type === 'WAITER' ? 'bg-blue-900/30 text-blue-400' : 'bg-yellow-900/30 text-yellow-400'}`}>
                      {opt.type}
                    </span>
                    {!opt.isActive && <span className="text-[10px] bg-gray-800 text-gray-500 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">Inactive</span>}
                  </div>
                  <div className="text-[var(--muted)] text-xs mt-0.5">Order: {opt.sortOrder} {opt.branchId ? '• Branch Specific' : '• Organization Wide'}</div>
                </div>
              </div>
              <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => startEdit(opt)} className="p-2 text-[var(--muted)] hover:text-[var(--accent)] transition-colors">Edit</button>
                <button onClick={() => handleDelete(opt.id)} className="p-2 text-[var(--muted)] hover:text-red-400 transition-colors"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
              </div>
            </div>
          ))
        )}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <form onSubmit={handleSave} className="relative card w-full max-w-md p-6 space-y-5 animate-in">
            ...
            <div>
              <label>Label</label>
              <input value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} placeholder="e.g. More Napkins" required />
            </div>

            <div>
              <label>Icon (Emoji)</label>
              <input value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} placeholder="Optional label" />
            </div>

            {isOrgAdmin && (
              <div>
                <label>Scope</label>
                <select value={form.branchId || ''} onChange={(e) => setForm({ ...form, branchId: e.target.value || null })}>
            ...
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="isActive" className="!mb-0">Active and visible to customers</label>
            </div>

            {saveError && <div className="bg-red-900/20 border border-red-800 text-red-400 px-3 py-2 text-sm">{saveError}</div>}

            <div className="flex gap-3 pt-2">
              <button type="button" onClick={() => setShowForm(false)} className="btn btn-secondary flex-1">Cancel</button>
              <button type="submit" disabled={saving} className="btn btn-primary flex-1">{saving ? 'Saving…' : 'Save Option'}</button>
            </div>
          </form>
        </div>
      )}
