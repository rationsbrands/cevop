...
export function OrgDetailPage() {
  ...
  async function load() {
    ...
    if (res.success) {
      ...
      setEditTrialEnd(res.data.trialEndsAt ? new Date(res.data.trialEndsAt).toISOString().slice(0, 10) : '');
    }
    setLoading(false);
  }
  ...
  async function save() {
    ...
    if (res.success) { setOrg((prev: any) => ({ ...prev, ...res.data })); setSuccess('Saved.'); }
    else setError(res.error || 'Failed to save');
    setSaving(false);
  }

  async function quickAction(action: 'suspend' | 'activate') {
    setSaving(true); setSuccess(''); setError('');
    const res = await api.post(`/api/ops/orgs/${orgId}/${action}`, {});
    if (res.success) { setOrg((prev: any) => ({ ...prev, ...res.data })); setEditStatus(res.data.planStatus); setSuccess(res.message); }
  ...
  return (
    <div className="space-y-6 animate-in max-w-4xl">
      {/* Breadcrumb */}
      <Link to="/orgs" className="text-xs text-[var(--muted)] hover:text-[var(--accent)] transition-colors">← All Organisations</Link>
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2 shrink-0">
          ...
          {(org.planStatus === 'suspended' || org.planStatus === 'trialing') && (
            <button onClick={() => quickAction('activate')} disabled={saving} className="btn btn-secondary btn-sm">Activate</button>
          )}
          <a href={adminDashUrl} target="_blank" rel="noreferrer" className="btn btn-secondary btn-sm">Open Dashboard ↗</a>
        </div>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card p-4"><p className="text-xs text-[var(--muted)] uppercase tracking-wider">Revenue</p><p className="font-display text-3xl text-[var(--accent)] mt-1">{formatPrice(org.stats?.totalRevenue ?? 0)}</p></div>
        <div className="card p-4"><p className="text-xs text-[var(--muted)] uppercase tracking-wider">Orders (30d)</p><p className="font-display text-3xl mt-1">{org.stats?.ordersLast30Days ?? 0}</p></div>
        <div className="card p-4"><p className="text-xs text-[var(--muted)] uppercase tracking-wider">Total Orders</p><p className="font-display text-3xl mt-1">{org._count?.orders ?? 0}</p></div>
        <div className="card p-4"><p className="text-xs text-[var(--muted)] uppercase tracking-wider">Tables</p><p className="font-display text-3xl mt-1">{org._count?.tables ?? 0}</p></div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Plan Management */}
        <div className="card p-5 space-y-4">
          <h2 className="font-semibold text-sm uppercase tracking-wider text-[var(--muted)] border-b border-[var(--border)] pb-2">Plan & Status</h2>
          <div>
            <label>Plan</label>
          ...
          <div>
            ...
            <textarea rows={3} value={editNotes} onChange={e => setEditNotes(e.target.value)} className="resize-none" placeholder="Notes visible to ops team only" />
          </div>
          {success && <p className="text-green-400 text-sm">{success}</p>}
          {error && <p className="text-red-400 text-sm">{error}</p>}
          <button onClick={save} disabled={saving} className="btn btn-primary w-full py-2 text-sm disabled:opacity-50">{saving ? 'Saving…' : 'Save Changes'}</button>
        </div>

        {/* Org Info */}
        <div className="card p-5 space-y-4">
          <h2 className="font-semibold text-sm uppercase tracking-wider text-[var(--muted)] border-b border-[var(--border)] pb-2">Organisation Info</h2>
          ...
          <InfoRow label="Verified At" value={org.verifiedAt ? new Date(org.verifiedAt).toLocaleString() : 'Not yet'} />
        </div>
      </div>
      ...
      {org.branches?.length > 0 && (
        <div className="card">
          <div className="card-header"><h2 className="font-semibold text-sm">Branches ({org.branches.length})</h2></div>
          <table>
            <thead><tr><th>Name</th><th>Slug</th><th>Users</th><th>Orders</th><th>Status</th></tr></thead>
            <tbody>
              {org.branches.map((b: any) => (
                <tr key={b.id}>
                  ...
                  <td><span className={`text-xs ${b.isActive ? 'text-green-400' : 'text-red-400'}`}>{b.isActive ? 'Active' : 'Inactive'}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {org.users?.length > 0 && (
        <div className="card">
          <div className="card-header"><h2 className="font-semibold text-sm">Admin Users</h2></div>
          <table>
            ...
            <tbody>
              {org.users.map((u: any) => (
                <tr key={u.id}>
                  ...
                  <td className="text-xs"><span className={`border px-2 py-0.5 ${u.role === 'ADMIN' ? 'border-[var(--accent)] text-[var(--accent)]' : 'border-blue-700 text-blue-400'}`}>{u.role}</span></td>
                  <td className="text-xs text-[var(--muted)]">{u.lastLoginAt ? new Date(u.lastLoginAt).toLocaleString() : 'Never'}</td>
                  <td><span className={`text-xs ${u.isActive ? 'text-green-400' : 'text-red-400'}`}>{u.isActive ? 'Active' : 'Inactive'}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
